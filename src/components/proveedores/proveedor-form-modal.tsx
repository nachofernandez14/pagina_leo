...
const VACIO: ProveedorInput = { nombre: "", telefono: null, notas: null };

export function ProveedorFormModal({ open, onClose, modo, proveedor, onGuardado }: Props) {
  const titleId = useId();
  const [campos, setCampos] = useState<ProveedorInput>(VACIO);
  ...
  useEffect(() => {
    ...
    setCampos(
      modo === "editar" && proveedor
        ? { nombre: proveedor.nombre, telefono: proveedor.telefono, notas: proveedor.notas }
        : VACIO,
    );
  }, [open, modo, proveedor]);
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    const res =
      modo === "crear"
        ? await crearProveedor(campos)
        : await actualizarProveedor({ ...campos, id: proveedor!.id });
    setLoading(false);
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4" role="presentation">
      ...
      >
        ...
        <form onSubmit={handleSubmit} className="mt-5 flex flex-col gap-4">
          <div>
            <label className={labelCls} htmlFor="pv-nombre">Nombre</label>
            <input
              id="pv-nombre"
              type="text"
              required
          ...
          <div>
            <label className={labelCls} htmlFor="pv-tel">
          ...
          <div>
            <label className={labelCls} htmlFor="pv-notas">
